...
class Worker {
    ...
    public constructor(start: int, end: int) {
        this.start_ = start;
        this.end_ = end;
    }
    ...
    step(): void {
        for (let index = this.start_; index < this.end_; ++index) {
            let agent = Simulator.Instance.agents_[index];
    ...
    doStep(): number {
        ...
        if (this.workers_ == null) {
            ...
            for (let block = 0; block < workersLength; ++block) {
                this.workers_[block] = new Worker(Math.trunc(block * numAgents / workersLength), Math.trunc((block + 1) * numAgents / workersLength));
            }
        }
